...
export function Sidebar() {
  ...
  return (
    <aside className="w-full md:w-64 border-r border-slate-800 bg-slate-950/70 backdrop-blur">
      <div className="p-4 border-b border-slate-800">
        <p className="text-sm text-slate-300 font-semibold">Silniki / tryby</p>
        <p className="text-xs text-slate-500">Wszystko edukacyjne, bez rekomendacji.</p>
      </div>
        ...
        {engines.map((engine) => (
          <Link
            ...
            className={`flex-1 md:flex-none px-4 py-3 text-sm transition hover:bg-slate-900/80 ${
              isActive(engine.id) ? 'bg-slate-900 text-brand-accent border-l-2 border-brand-primary' : 'text-slate-200'
            }`}
          >
      ...
      <div className="p-4 border-t border-slate-800 space-y-2">
        <div>
          <p className="text-xs text-slate-400 uppercase tracking-wide">Raporty</p>
          <p className="text-[11px] text-slate-500">Skróty do wpisów Founder i tygodnia.</p>
        </div>
            ...
            return (
              <Link
                ...
                className={`block px-3 py-2 rounded-lg text-sm transition hover:bg-slate-900/80 ${
                  active ? 'bg-slate-900 text-brand-accent border border-brand-primary/30' : 'text-slate-200'
                }`}
              >
